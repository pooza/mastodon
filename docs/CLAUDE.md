...
- ⚠⚠ **`air` は既に食い違っている。揃えるなら 2 つの軸を同時に見ること。**
  **揃えるかどうか自体が未判断**だが、着手するなら下の表を先に埋める（2026-08-31 実測）:

  | | 表示ラベル | 投稿に載るタグ |
  | --- | --- | --- |
  | このフォーク | `air` なら無条件（[tagset_dropdown.tsx:107](../app/javascript/mastodon/features/compose/components/tagset_dropdown.tsx)） | `air` なら無条件（[compose.js:437](../app/javascript/mastodon/reducers/compose.js)） |
  | ダイスキー | 🔴 **`livecure` が真のときだけ**（`WidgetTagset.vue:107`） | 🔴 **`air` なら無条件**（同 `:186`） |

  つまり **①クライアント間**（このフォーク vs ダイスキーのラベル）だけでなく、
  🔴 **②ダイスキー内部でラベルとタグ値が食い違っている**。`air: true, livecure: false` の枠では
  **ラベルに「エア番組」が出ないのに `user_tags` には入る**。⚠⚠ ダイスキーの確認ダイアログは
  ラベルを再利用しているので、**利用者が見た文字列と実際に送られるタグが一致しない**。
  ⚠ **このフォークは①②とも一貫している**（ラベルもタグも `air` 無条件）ので、片方だけ見て
  「Mastodon に合わせる」と決めると、②が残ったままになる
